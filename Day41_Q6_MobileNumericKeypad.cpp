class Solution {
  ...
  public:
    ...
    long long solve(int i, int j, int n, int track, string temp, vector<vector<vector<long long>>>& dp){
        ...
        if(j-1>=0) left=solve(i, j-1, n, track+1, temp + ar[i][j-1], dp);
        if(j+1<3)  right=solve(i, j+1, n, track+1, temp + ar[i][j+1], dp);
        if(i-1>=0) up=solve(i-1, j, n, track+1, temp + ar[i-1][j], dp);
        if(i+1<4)  down=solve(i+1, j, n, track+1, temp + ar[i+1][j], dp);
